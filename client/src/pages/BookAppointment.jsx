...
const Appointment = () => {
  ...
  // 📌 Prefill department/doctor from query params
  useEffect(() => {
    const prefillDept = params.get("department");
    const prefillDoctor = params.get("doctor");
  ...
  const handleChange = (e) => {
    ...
    // prevent typing invalid date in inputs
    if (name === "dob") {
      const today = new Date().toISOString().split("T")[0];
      if (value > today) return; // block future DOB
    }
    if (name === "date") {
      const today = new Date().toISOString().split("T")[0];
      if (value < today) return; // block past appointments
    }
  ...
  const validateStep = () => {
    ...
    if (step === 2) {
      return formData.department && formData.doctor && formData.reason;
    }
    if (step === 3) {
  ...
  return (
    <div className="bg-blue-50 min-h-screen p-4">
      ...
      <div className="bg-blue-600 text-white rounded-lg py-6 px-4 text-center shadow-md mb-6">
        ...
        <p className="mt-2">
          Fast, easy, and secure online booking with our doctors.
        </p>
      </div>
      ...
      <div className="flex justify-center items-center mb-8 space-x-4 text-sm">
        {steps.map((s, idx) => (
          ...
          >
            ...
            {idx < steps.length - 1 && (
              <span className="w-6 border-b border-gray-300 mx-2"></span>
            )}
          </div>
        ))}
      </div>
